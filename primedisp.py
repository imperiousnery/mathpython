...
def analyze_prime_distances(n):
    ...
    # Arredondar as diferenças para o número inteiro mais próximo e remover valores repetidos
    unique_prime_diffs = np.unique(np.round(prime_diffs).astype(int))

    # Exibir o resultado
    print(f"\nAnálise para {n} números:")
    print("Coeficientes reais:", real_coeffs)
    print("Coeficientes ajustados:", adjusted_coeffs)
    print("Diferenças entre números primos (antes do arredondamento):", prime_diffs)
    print("Diferenças entre números primos (após o arredondamento e remoção de valores repetidos):", unique_prime_diffs)
