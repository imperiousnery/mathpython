...
def generate_8bit_music(duration, output_file):
    ...
    for _ in range(int(duration * 4)):  # Cada unidade de tempo é de 0.25 segundos
        ...
        # Adicionar acordes e melodias ao arquivo MIDI
        for chord, note in zip(chords, melody):
            midi_file.addChord(track, 0, [note], time, 0.25)  # Duração de cada acorde é de 0.25 segundos
            midi_file.addNote(track, 0, note, time, 0.25)  # Duração de cada nota da melodia é de 0.25 segundos
            time += 0.25
