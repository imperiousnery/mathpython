...
while True:
    try:
        search_engine_choice = input("Escolha o mecanismo de busca (google, duckduckgo, yahoo): ").lower()

        if search_engine_choice not in ["google", "duckduckgo", "yahoo"]:
        ...
        with ThreadPoolExecutor() as executor:
            if search_engine_choice == "google":
                future = executor.submit(search, query, num=10, stop=10, user_agent=get_random_user_agent())
                ...
                future = executor.submit(requests.get, f"https://search.yahoo.com/search?p={query}&b=30&n=40")
        ...
        if search_engine_choice == "duckduckgo":
            ...
            if search_engine_choice == "yahoo":
                ...
                results = soup.select(".searchCenterMiddle li a")
                for i, result in enumerate(results, start=30):
        ...
        content_summary = get_site_summary(selected_result['link'] if search_engine_choice == "duckduckgo" else selected_result)
